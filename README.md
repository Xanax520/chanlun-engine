...
4. （可选）条件选股：Ctrl+F 新建「条件选股公式」，粘贴 `tdx_dll\formulas\缠论二三买选股公式.txt` 内容保存；「功能 → 选股器 → 条件选股」选择该公式即可全市场扫描二买/三买（全市场 5220 只 × 30 分钟周期 × 5 天窗口，端到端约 13 秒；同条件换 5 分钟周期（平均约 15000 根K线、一天窗口）端到端约 36 秒；实测 12.52 秒 / 36 秒；测试平台 i7-9700F / 8核 / 16GB / 通达信64位）。
